...
def import_plays_datas(season='2020-2021', file_pattern='combined-stats'):
    season = season
    ...
    datas = pd.read_csv('%s%s%s' % (dir_datas, '\\', file_data))
...
def import_players_datas(season2='2020-2021', file_pattern2='playerSummary'):
    ...
    datas2 = pd.read_csv('%s%s%s' % (dir_datas2, '\\', file_data2))
